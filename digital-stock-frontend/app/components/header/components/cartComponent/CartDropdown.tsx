...
const CartDropdown = ({ cartItems, onClose }: CartItemProps) => {
  ...
  const showContents = () => {
    if (user) {
      return (
        <div className="py-6 overflow-scroll no-scrollbar mb-36">
      ...
      return (
        <div className="flex py-16 flex-col items-center justify-center">
          <h4 className="text-lg font-semibold pb-5 text-gray-600">
            Sign in to see shopping cart
          </h4>
          <AuthForm>
            <div className="bg-blue-500 text-white font-semibold rounded-xl py-3 px-9">
              Sign in
            </div>
          </AuthForm>
        </div>
  ...
  return (
    <div className="w-[500px] max-w-xl bg-white shadow-lg relative ml-auto h-screen flex flex-col">
      <div className="p-3">
        ...
        <p className="flex-1 border-b text-sm font-bold text-gray-400 pb-4">
          Items: {itemsAmount}
        </p>
      </div>
      <div className="overflow-auto flex-grow p-6">
        <div className="h-full">{showContents()}</div>
      </div>
      <div className="p-6 border-t bg-white">
        ...
        {itemsAmount > 0 ? (
          <button
            className="mt-3 text-sm font-semibold px-6 py-3 w-full bg-gray-300 hover:bg-gray-400 transition-all duration-300 text-white rounded-md tracking-wide"
